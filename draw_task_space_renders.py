...
def compute_task_error(pose_data, config_data, seg_length_itrs, eps, config_data_2 = None):
    ...
    for itr in range(1, len(seg_length_itrs)):
        ...
        if high_shear_stiffness == True:
            ...
            plot_pose_itr, = ax.plot([], [], 'r-o', label= 'Dynamic model - 2 segment (sparsified strains)')
        else:
            plot_pose_itr, = ax.plot([], [], 'r-o', label= f'Dynamic model - {num_segments} segment (no noise)')
            if config_data_2 is not None:
                plot_pose_itr_2, = ax.plot([], [], '-o', color='C1', label= f'Dynamic model - {num_segments} segment (w/ noise)')
